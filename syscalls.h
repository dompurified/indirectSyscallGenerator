...
#include <xorstr/xorstr.hpp> // change path
...
#define i(func,...) DWORD sysnum_Nt##func; void* stub_Nt##func;
...
#define i(func,...) {\
auto ntdll_func = GET_FUNC_ADDR(ntdll, xorstring_or_string("Nt" #func));\
if (!ntdll_func) return 0; \
uint8_t sig1[] = { 0x4C, 0x8B, 0xD1, 0xB8 }; \
if (memcmp(ntdll_func, sig1, sizeof(sig1)) != 0) \
return 0; \
auto sysnum = *(DWORD*)(ntdll_func + 0x4); \
auto stub = ntdll_func + 0x12; \
uint8_t sig2[] = { 0x0F, 0x05, 0xC3 }; \
if (memcmp(ntdll_func + 0x12, sig2, sizeof(sig2)) != 0) \
return 0; \
sysnum_Nt##func = sysnum; \
stub_Nt##func = (void*)stub; \
}
#define j(ret, convention, func,...) i(func)
